...
const TestViewWithStatic = (viewModel: ViewModelWithStatic) => (
  <div>
    Message1: {viewModel.message}, message2: {viewModel.static}
  </div>
);

it(
  'should render initial value',
  setupAppContainerAndRender(TestView, new ViewModel(), container => {
...
it(
  'can mix text, static variables and observable inside a single element',
  setupAppContainerAndRender(
    TestViewWithStatic,
    new ViewModelWithStatic(),
    (container, viewModel) => {
      viewModel.message.setValue('updated');
      expect(container.innerHTML).toBe(
        '<div>Message1: updated, message2: static string</div>'
      );
    }
  )
);
