    ...
    template(): HtmlDefinition {
        return <section class="js-debugger">
                    <header>
                        ...
                        <b style="color: #FFFFFF;">JohnSmith Debugger</b>
                        <span style="flex: 1"/>
                        <span>v{JS_VERSION}</span>
                    </header>
                    <ul>
                        <List view={error => {
                            return <li
                                _mouseenter={() => error.highlightErrorElement() }
                                _mouseleave={() => error.unHighlightErrorElement() }>
                                <section class="js-debugger-error-index">#{error.index}</section>
                                <section class="js-debugger-error-message">{error.message}</section>
                            </li>;
