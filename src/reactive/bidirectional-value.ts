...
export type ChangeHandler<T> = (
  value: T
) => boolean | null | undefined | void | ChangeHandlerResult<T>;

export class BidirectionalValue<T>
  extends ObservableValue<T>
  implements BidirectionalListenable<T>
{
  constructor(
    private changeHandler: ChangeHandler<T>,
    value: T
  ) {
    super(value);
  }
  ...
  public requestUpdate(newValue: T) {
    ...
    if (
      ...
      handlerResult !== false &&
      handlerResult.newValue !== undefined
    ) {
