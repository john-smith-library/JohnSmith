...
var FileViewModel = function(file){
    this.children = js.observableList();
    this.name = file.name;
...
var FileSystemViewModel = function(fileSystem){
    this.fileTree = js.observableList();
...
var FileView = function(){
    ...
    this.init = function(dom, fileViewModel){
        if (fileViewModel.isDirectory) {
            dom('.item').$.addClass("directory");
        }

        dom('.name').observes(fileViewModel.name);
        dom('.children').observes(fileViewModel.children, FileView);

        dom('a').on('click').react(fileViewModel.showFullPath);
    };
};

var FileTreeView = function(){
    this.template = "#FileSystemTemplate";
    this.init = function(dom, fileSystemViewModel){
        dom('#treeRoot').observes(fileSystemViewModel.fileTree, FileView);
    };
};
...
js.dom('#fileTree').render(FileTreeView, fileSystemViewModel);